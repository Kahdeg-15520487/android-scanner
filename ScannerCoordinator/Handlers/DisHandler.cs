namespace ScannerCoordinator.Handlers
{
    ...
    public class DisHandler : IDisHandler, IDisposable
    {
        ...
        public DisHandler(IConfiguration configuration)
        {
            ...
            consumer.Received += async (ch, ea) =>
            {
                ...
                else
                {
                    ...
                    if (_sockets.TryGetValue(workerId, out var ws) && ws.State == WebSocketState.Open)
                    {
                        ...
                        var val = JsonConvert.SerializeObject(msg);
        ...
        public async Task PushAsync(HttpContext context, WebSocket webSocket)
        {
            ...
            Console.WriteLine(_sockets.ContainsKey(clientId));
        }
        ...
        public async Task DisconnectAsync(HttpContext context, WebSocket webSocket)
        {
            var disconnectedSocket = _sockets.FirstOrDefault(kvp => kvp.Value == webSocket);
            _sockets.TryRemove(disconnectedSocket.Key, out _);
        }
    }
}
